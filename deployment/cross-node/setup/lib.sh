...
function wait_for_elasticsearch {
	...
	local -a args=( '-k' '-s' '-D-' '-m15' '-w' '%{http_code}' "https://${elasticsearch_host}:9200/" )
...
function wait_for_builtin_users {
	...
	local -a args=( '-k' '-s' '-D-' '-m15' "https://${elasticsearch_host}:9200/_security/user?pretty" )
...
function check_user_exists {
	...
	local -a args=( '-k' '-s' '-D-' '-m15' '-w' '%{http_code}'
